...
export default function LandingScanner() {
  ...
  const startCamera = () => {
    setIsScanning(true);
    // Let the DOM update so qr-reader is visible
    setTimeout(() => {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Restart camera after failed file scan if we were scanning
      if (isScanning) {
        startCamera();
      }
  ...
  return (
    <div className="relative min-h-[100dvh] overflow-hidden flex flex-col items-center justify-between bg-[#f4f8f0]">
      ...
      {/* ── Falling Leaves (Only when not scanning) ───────────────── */}
      {!isScanning && (
      ...
      {/* ── Top Header (Only when not scanning) ───────────────────── */}
      {!isScanning && (
        <motion.div
          initial={{ y: -50, opacity: 0 }}
          animate={{ y: 0, opacity: 1 }}
          transition={{ duration: 0.8, ease: "easeOut" }}
          className="relative z-[3] w-full flex flex-col items-center pt-16 pb-4 px-6"
        >
          <motion.div
            animate={{ scale: [1, 1.06, 1] }}
            transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}
            className="w-24 h-24 rounded-full border-4 border-moss-canopy bg-white shadow-[0_6px_30px_rgba(75,107,58,0.3)] overflow-hidden mb-6"
          >
          ...
          <p className="font-sans text-sm text-slate-bark mt-2 text-center max-w-[280px] leading-relaxed">
            Welcome to the Citizen Portal. Please scan a tree tag to view its life record or report an issue.
          </p>
        </motion.div>
      )}
      ...
      {/* ── QR Scanner View ───────────────────────────────────────── */}
      {isScanning ? (
        <div className="w-full h-full flex flex-col flex-1 z-10 bg-black items-center justify-center relative">
          <div className="w-full max-w-md w-full relative">
            ...
            <div className="absolute top-4 left-0 right-0 flex justify-center z-20">
              {scanError && (
                <div className="bg-red-500 text-white font-sans text-sm font-semibold px-6 py-2 rounded-full shadow-lg">
          ...
          <div className="absolute bottom-10 w-full flex justify-center z-20">
            <button
              onClick={stopCamera}
              className="bg-white/20 backdrop-blur-md border border-white/40 text-white font-sans font-medium text-sm px-6 py-3 rounded-full hover:bg-white/30 transition-all"
            >
              Cancel Scanning
            </button>
          </div>
        ...
        <motion.div
          ...
          className="relative z-[3] flex flex-col items-center gap-5 px-6 mb-auto mt-auto"
        >
          <button
            onClick={startCamera}
            className="flex items-center gap-2 bg-moss-canopy hover:bg-moss-canopy-dark text-white font-display text-lg tracking-[0.1em] uppercase px-8 py-4 rounded-full shadow-[0_6px_24px_rgba(75,107,58,0.35)] transition-all active:scale-95"
          >
      ...
      {/* ── Bottom Actions (Only when not scanning) ───────────────── */}
      {!isScanning && (
        <motion.div
          initial={{ y: 50, opacity: 0 }}
          animate={{ y: 0, opacity: 1 }}
